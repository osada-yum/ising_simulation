module union_find_m
  ...
  implicit none
  type union_find
     integer(int64), allocatable :: par_(:), size_(:)
     integer(int64) :: uf_size_
   contains
...
contains
  subroutine init_uf(this, n)
    ...
    if (allocated(this%par_)) then
       if (this%uf_size_ /= n) then
          this%uf_size_ = n
          block
            integer(int64), allocatable :: new_par(:), new_size(:)
  ...
  impure recursive integer(int64) function root_uf(this, i) result(res)
    class(union_find), intent(inout) :: this
