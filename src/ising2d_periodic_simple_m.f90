module ising2d_periodic_simple_m
  use, intrinsic :: iso_fortran_env
  use msmt19937
  implicit none
  private
  character(len=*), parameter :: version = "simple"
...
contains
  ...
  impure subroutine local_flip(x, y)
    ...
    delta_e = calc_delta_local_energy(nearest_states(1:4), ising2d(x, y))
    if (delta_e > 0) then
       if (.not. (rnds(x, y) < exp(- beta * delta_e))) &
            & return
    end if
    !> delta_e <= 0 .or. rnds(x, y) < exp(- beta * delta_e)
    ising2d(x, y) = - ising2d(x, y)
  end subroutine local_flip
  !> calc_delta_local_energy: Calculate the differential of the local energy at center.
  !> @param: nearests States of nearest spin of `c`.
  !> @param: c A state of center spin.
  pure integer(int32) function calc_delta_local_energy(nearests, c) result(res)
    integer(int32), intent(in) :: nearests(4), c
    res = 2 * c * sum(nearests(:))
  end function calc_delta_local_energy
...
end module ising2d_periodic_simple_m
