module ising2d_periodic_dual_lattice_locali_probtab_globpar_int8_m
  use, intrinsic :: iso_fortran_env
  use msmt19937
  implicit none
  private
  character(len=*), parameter :: version = "dual_lattice_locality_probtable_globalparam_int8"
  ...
  integer(int8), allocatable :: ising2d_even(:, :)
  integer(int8), allocatable :: ising2d_odd(:, :)
...
contains
  ...
  impure subroutine local_flip_even(ising2d_even, ising2d_odd, x, y, magne_global, energy_global)
    integer(int8), intent(inout) :: ising2d_even(nx, ny / 2)
    integer(int8), intent(in) :: ising2d_odd(nx, ny / 2)
    integer(int64), intent(in) :: x, y
    integer(int64), intent(inout) :: magne_global, energy_global
    integer(int8) :: nearest_states(nd)
    integer(int64) :: rx, lx, uy, dy
    integer(int8) :: delta_e
    real(real64) :: prob
    !> left
    lx = x - 1
    if (lx < 1) lx = nx
    ...
    delta_e = 2_int8 * ising2d_even(x, y) * sum(nearest_states(1:4))
    if (delta_e > 0_int8) then
       prob = probability_table(delta_e)
       if (.not. (grnd() < prob)) &
            & return
    end if
    !> delta_e <= 0 .or. grnd() < exp(- beta * delta_e)
    ising2d_even(x, y) = - ising2d_even(x, y)
    magne_global = magne_global + 2_int8 * ising2d_even(x, y)
  ...
  !> @param x A x-position of the spin.
  !> @param y A y-position of the spin.
  impure subroutine local_flip_odd(ising2d_odd, ising2d_even, x, y, magne_global, energy_global)
    integer(int8), intent(inout) :: ising2d_odd(nx, ny / 2)
    integer(int8), intent(in) :: ising2d_even(nx, ny / 2)
    integer(int64), intent(in) :: x, y
    integer(int64), intent(inout) :: magne_global, energy_global
    integer(int8) :: nearest_states(nd)
    integer(int64) :: rx, lx, uy, dy
    ...
    delta_e = 2_int8 * ising2d_odd(x, y) * sum(nearest_states(1:4))
    if (delta_e > 0_int8) then
       prob = probability_table(delta_e)
       if (.not. (grnd() < prob)) &
            & return
    ...
    magne_global = magne_global + 2_int8 * ising2d_odd(x, y)
    energy_global = energy_global + delta_e
...
end module ising2d_periodic_dual_lattice_locali_probtab_globpar_int8_m
